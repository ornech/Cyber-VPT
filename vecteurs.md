...
| `d2` | Entropie | payload | `H(payload) / Hmax` | repérer obfuscation, encodage, shellcode, charge inhabituelle |
...
### 4.2. `d2` — entropie
`d2` mesure le désordre informationnel du contenu manipulé.

Exemple :

$$d2 = \frac{H(payload)}{H_{max}}$$

avec `Hmax = 8` pour une distribution sur octets.

Une entropie élevée n'est pas un aveu d'attaque. Elle signale un contenu dense, chiffré, obfusqué ou atypique. C'est déjà suffisant pour compliquer le tri.
